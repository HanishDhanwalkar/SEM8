...
% Updated Experimental Evaluation Section
...
\end{itemize}
 ...
 \begin{figure}
     \centering
     \includegraphics[width=0.5\linewidth]{images/k=50.jpeg}
...
\STATE $\theta = 1/(50k)$ \hfill \COMMENT{Improved convergence parameter}
\FOR{$i = 1$ to $T$}
    \STATE $S \leftarrow \text{Enhanced-LS}(A', k, S, \theta)$
    \STATE $\epsilon_i = \|A' - SS^\dagger A'\|_F^2$
    \IF{$\epsilon_{i-1} - \epsilon_i < \theta \cdot \epsilon_{i-1}/10$}
...
\begin{theorem}[Improved Approximation Ratio]
...
\begin{lemma}[Enhanced Column Selection]
...
\begin{lemma}[Enhanced Success Probability]
...
\begin{lemma}[Enhanced Running Time]
...
\begin{lemma}[Enhanced Perturbation Analysis]
